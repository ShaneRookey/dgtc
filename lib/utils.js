...
export const updateLeague = async (league_id, newName, newRegion) => {
    try {
        const res = await fetch(
            `http://localhost:3000/api/league/${league_id}`,
            {
...
export const addPlayer = async (firstName, lastName, team_id) => {
    try {
        const res = await fetch("http://localhost:3000/api/player/add", {
            method: "POST",
            headers: {
                "Content-type": "application/json",
            },
...
export const getPlayersByTeamId = async (team_id) => {
    try {
        const res = await fetch(
            `http://localhost:3000/api/team/${team_id}/players`,
            {
                cache: "no-store",
            }
        );
